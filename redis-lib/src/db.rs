...
/// allow orderly cleanup of the `db` by signalling the background purge task to shut down when
...
struct Shared {
    ...
    /// The background task waits on this to be notified, then checks for expired values or the
    /// shutdown signal.
